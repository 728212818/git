...
CENTER_MARK_SIZE = 18
VISION_UART_ID = UART.UART2
VISION_UART_BAUD = 115200
VISION_UART_TX_PIN = 11
VISION_UART_RX_PIN = 12
...
def send_vision_error(uart, err_x, err_y, valid):
    err_x = clamp_int16(err_x)
    err_y = clamp_int16(err_y)
...
def detection():
    ...
    # 初始化并配置sensor
    fpioa = FPIOA()
    fpioa.set_function(VISION_UART_TX_PIN, FPIOA.UART2_TXD)
    fpioa.set_function(VISION_UART_RX_PIN, FPIOA.UART2_RXD)
    vision_uart = UART(VISION_UART_ID, VISION_UART_BAUD, bits=8, parity=0, stop=1)
        ...
        while  True:
            with ScopedTiming("total",debug_mode > 0):
                ...
                if rgb888p_img.format() == image.RGBP888:
                    ...
                    osd_img.clear()
                    screen_cx = DISPLAY_WIDTH // 2
                    screen_cy = DISPLAY_HEIGHT // 2
                    osd_img.draw_line(screen_cx - CENTER_MARK_SIZE, screen_cy, screen_cx + CENTER_MARK_SIZE, screen_cy, color=(0, 255, 0))
                    osd_img.draw_line(screen_cx, screen_cy - CENTER_MARK_SIZE, screen_cx, screen_cy + CENTER_MARK_SIZE, color=(0, 255, 0))
                    osd_img.draw_string_advanced(screen_cx + 8, screen_cy + 8, 24, "CENTER", color=(0, 255, 0))
                    target_valid = 0
                    send_err_x = 0
                    send_err_y = 0
                    if det_boxes:
                        for det_boxe in det_boxes:
                            ...
                            w = disp_x2 - disp_x1
                            h = disp_y2 - disp_y1
                            rect_cx = (disp_x1 + disp_x2) // 2
                            rect_cy = (disp_y1 + disp_y2) // 2
                            err_x = rect_cx - screen_cx
                            err_y = rect_cy - screen_cy
                            if target_valid == 0:
                                send_err_x = err_x
                                send_err_y = err_y
                                target_valid = 1
                            osd_img.draw_rectangle(disp_x1, disp_y1, int(w), int(h), color=color_four[det_boxe[0]][1:])
                            osd_img.draw_line(rect_cx - CENTER_MARK_SIZE, rect_cy, rect_cx + CENTER_MARK_SIZE, rect_cy, color=(255, 0, 0))
                            osd_img.draw_line(rect_cx, rect_cy - CENTER_MARK_SIZE, rect_cx, rect_cy + CENTER_MARK_SIZE, color=(255, 0, 0))
                            label = labels[det_boxe[0]]
                            score = str(round(det_boxe[1],2))
                            osd_img.draw_string_advanced(disp_x1, disp_y1 - 50, 32, label + " " + score, color=color_four[det_boxe[0]][1:])
                            osd_img.draw_string_advanced(rect_cx + 8, rect_cy + 8, 24, "err:" + str(err_x) + "," + str(err_y), color=(255, 0, 0))
                    send_vision_error(vision_uart, send_err_x, send_err_y, target_valid)
